...
b_knight_pst = np.array([[ -66, -53, -75, -75, -10, -55, -58, -70],
...
b_bishop_pst = np.array([[ -59, -78, -82, -76, -23,-107, -37, -50],
...
b_rook_pst = np.array([[  35,  29,  33,   4,  37,  33,  56,  50],
...
b_queen_pst = np.array([[   6,   1,  -8,-104,  69,  24,  88,  26],
...
b_king_pst = np.array(   [[4,  54,  47, -99, -99,  60,  83, -62],
...
def calc_piece_activity(board=chess.Board()):
    ...
    for idx, char in enumerate(board_epd):
        if char.isalpha():
            if char.isupper():
                sum += pst_dic[char][find_piece_index(board_epd, idx)] + piece_vals[char]
    ...
    for idx, char in enumerate(reversed_board_epd):
        if char.isalpha():
            if char.isupper():
                sum -= (
                    pst_dic[char][find_piece_index(reversed_board_epd, idx)]
                    + piece_vals[char]
                )
    # print(f"Score: {sum}")
    return sum
